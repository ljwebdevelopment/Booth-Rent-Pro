...
function renderRenters() {
  ...
  visibleRenters.forEach((renter) => {
    ...
    rentersListElement.appendChild(createRenterCard({ ...renter, computedStatus: summary?.status || 'due' }, openRenterDrawer));
  });
}
...
async function refreshAllRenterSummaries(uid, renterRows) {
  const summaryEntries = await Promise.all(
    renterRows.map(async (renter) => [renter.id, await refreshRenterMonthSummary(uid, renter)])
  );
...
async function openRenterDrawer(renter) {
  ...
  const { currentUser } = getUiState();
  if (currentUser) await billing.ensureMonthlyCharge(currentUser.uid, renter);
...
async function handleCreateChargeSubmit(event) {
  ...
  try {
    ...
    await refreshAllRenterSummaries(currentUser.uid, getUiState().renters);
    renderRenters();
  } catch (error) {
